...
public class UserAction extends BaseAction {
	...
	public void getTree() {
		...
		List<CategoryTreeBean> treeList = new ArrayList<CategoryTreeBean>();
		treeList = userService.getTreeList(userRole);
		Map<String, CategoryTreeBean> map = new LinkedHashMap<String, CategoryTreeBean>();
		Map<String, CategoryTreeBean> mapCompare = new LinkedHashMap<String, CategoryTreeBean>();
		for (CategoryTreeBean t : treeList) {
			map.put(t.getId(), t);
			mapCompare.put(t.getId(), t);
		}
		
		Iterator it = map.keySet().iterator();
		while (it.hasNext()) {
			CategoryTreeBean category = map.get(it.next());
			
			if (category.getId() != null && !"null".equals(category.getId()) && mapCompare.containsKey(category.getParent_id())) {// 第一级节点
				CategoryTreeBean categoryCompare = mapCompare.get(category.getParent_id());
				if (categoryCompare.getChildren() != null) {
					categoryCompare.getChildren().add(category);
				} else {
					List<CategoryTreeBean> childrens = new ArrayList<CategoryTreeBean>();
					childrens.add(category);
					categoryCompare.setChildren(childrens);
				}
				mapCompare.remove(category.getId());
			}
		}
		List<CategoryTreeBean> newList = new ArrayList<CategoryTreeBean>();
		Iterator i = mapCompare.keySet().iterator();
		while (i.hasNext()) {
			newList.add((CategoryTreeBean) mapCompare.get(i.next()));
		}
		writeJson(newList);
	}
	...
	public void updateUserAuth() throws JsonGenerationException,
	...
	public void updatePwd() throws JsonGenerationException,
			JsonMappingException, IOException {
		ObjectMapper mapper = new ObjectMapper();
		Map<String, String> map = new HashMap<String, String>();
		HttpSession session = getRequest().getSession();
		...
		operateHistoryService.insertOH(getRequest(), (String) session
				.getAttribute("userId"), "9", msg, msg == null ? 1 : 0);
		this.printData(getResponse(), mapper.writeValueAsString(map));
	}

	public void listUser() throws JsonGenerationException,
			JsonMappingException, IOException {
		ObjectMapper mapper = new ObjectMapper();
	...
	public void listRole() throws JsonGenerationException,
			JsonMappingException, IOException {
		ObjectMapper mapper = new ObjectMapper();
		Map<String, String> condition = new HashMap<String, String>();
	...
	public void deleteRole() throws JsonGenerationException,
			JsonMappingException, IOException {
		HttpSession session = getRequest().getSession();
		ObjectMapper mapper = new ObjectMapper();
		Map<String, String> map = new HashMap<String, String>();
		...
		operateHistoryService.insertOH(getRequest(), (String) session
				.getAttribute("userId"), "15", msg, msg == null ? 1 : 0);
		this.printData(getResponse(), mapper.writeValueAsString(map));
	}

	public void deleteUser() throws JsonGenerationException,
			JsonMappingException, IOException {
		HttpSession session = getRequest().getSession();
		String msg = null;
		ObjectMapper mapper = new ObjectMapper();
		...
		operateHistoryService.insertOH(getRequest(), (String) session
				.getAttribute("userId"), "12", msg, msg == null ? 1 : 0);
		this.printData(getResponse(), mapper.writeValueAsString(map));
	}

	public void editRole() throws JsonGenerationException,
			JsonMappingException, IOException {
		HttpSession session = getRequest().getSession();
		ObjectMapper mapper = new ObjectMapper();
		Map<String, String> map = new HashMap<String, String>();
		...
		if (msg == null) {
			if (ToolUtils.isNotBlank(roleId)) {// roleId 不为空，编辑角色
				...
				operateHistoryService.insertOH(getRequest(),
						(String) session.getAttribute("userId"), "13", msg,
						msg == null ? 1 : 0);
			} else {// 添加用户
				if (!userService.isExitRole(roleName)) {// 检查是否存在重复角色名
					try {
				...
				operateHistoryService.insertOH(getRequest(),
						(String) session.getAttribute("userId"), "14", msg,
						msg == null ? 1 : 0);
			}
		}
		if (msg == null) {
	...
	public void editUser() throws JsonGenerationException,
			JsonMappingException, IOException {
		HttpSession session = getRequest().getSession();
		ObjectMapper mapper = new ObjectMapper();
		Map<String, String> map = new HashMap<String, String>();
		...
		String userRole = getRequest().getParameter("userRole");
		String msg = null;
		if (!ToolUtils.isNotBlank(userName) || !ToolUtils.isNotBlank(userPwd)
				|| !ToolUtils.isNotBlank(userRole)) {
			msg = "参数有误!";
		}
		int role = 1;
		if (msg == null) {
		...
		if (msg == null) {
			if (!StringUtils.isNotBlank(userId) && !"null".equals(userId)) {// 添加用户
				...
				operateHistoryService.insertOH(getRequest(),
						(String) session.getAttribute("userId"), "10", msg,
						msg == null ? 1 : 0);
			} else {// 编辑用户
				...
				operateHistoryService.insertOH(getRequest(),
						(String) session.getAttribute("userId"), "11", msg,
						msg == null ? 1 : 0);
			}
		}
		if (msg == null) {
	...
	public void getRole() throws JsonGenerationException, JsonMappingException,
			IOException {
		String msg = null;
